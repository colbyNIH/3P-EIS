...
write_fits_table = 0; % if set to 1, saves the most recent fits to an excel table for external analysis such as in R
...
figFits = uifigure('Name','Fit Data','Units', 'Normalized', 'Position',[0.6, 0.15, 0.25, 0.3]);
...
for i = 1:n_files
    ...
    for h = 1:length(meas_idx)
        ...
        try
            ...
            remove_max_f = 0;
            max_f = 1000; % [Hz] frequencies above this value are removed
            min_f = 0; % [Hz] frequencies below this value are removed
            if remove_max_f
                fi_data = data.frequency_info{:};
            ...
            uit.Data = table(p_name,p_actual,pg_absEr,pg_absZr,pg_absnone);
            drawnow();
            ...
            legend({'measured','theory','fit w/ pipette','fit w/out pipette'},'Location','eastoutside')
            ...
            legend({'measured','theory','fit w/ pipette'},'Location','eastoutside')
            set(gca,'FontSize',figure_fs)
